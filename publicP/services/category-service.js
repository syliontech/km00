    ...
    async loadAllCategories() {
        try {
            ...
            if (window.CONFIG && window.CONFIG.FORCE_MOCK_DATA) {
                ...
                console.log('加载了', window.allCategories.length, '条模拟分类数据');
            ...
            const { data, error } = await this.supabaseClient
                .from(this.tableName)
                .select('category_pkey, code, parent_code, name, level, is_leaf, purpose_name, epurpose_name, category_name, ecategory_name, subcategory_name, esubcategory_name, detail_name, edetail_name, created_at, updated_at')
            ...
            if (error) {
                ...
                // 如果数据库查询失败，尝试使用模拟数据
                if (window.CONFIG && window.CONFIG.mockCategories) {
    ...
    async getCategories() {
        try {
            ...
            if (this.supabaseClient) {
                ...
                // 将数据存储在全局变量中
                window.allCategories = data || [];
                console.log('成功获取分类数据:', window.allCategories.length, '条记录');
                
                return window.allCategories;
            } else {
                console.warn('没有可用的 Supabase 客户端，尝试使用模拟数据');
                
                // 如果没有 Supabase 客户端，尝试使用模拟数据
                if (window.CONFIG && window.CONFIG.mockCategories) {
    ...
    async getAllCategories() {
        try {
            ...
            if (window.allCategories && window.allCategories.length > 0) {
                console.log('使用预加载的分类数据，共', window.allCategories.length, '条记录');
                return window.allCategories;
            }
            ...
            if (this.supabaseClient) {
                ...
                if (error) {
                    console.error('从数据库获取分类数据失败:', error);
                    throw new Error(`从数据库获取分类数据失败: ${error.message}`);
                }
                
                if (!data || data.length === 0) {
                    console.warn('数据库中没有分类数据');
                    return [];
                }
                
                // 将数据存储在全局变量中
                window.allCategories = data;
                console.log('成功从数据库获取分类数据，共', data.length, '条记录');
                
                return data;
            } else {
                console.warn('没有可用的 Supabase 客户端，无法从数据库获取分类数据');
