...
ProductSystem.SpecsManager = (function() {
    // 私有变量
    let productAttributes = []; // 当前显示的产品属性数组
    let allAttributeRecords = {}; // 存储所有属性记录的历史，用于产品明细显示
    let specsList = null;
    ...
    let hasDeletePermission = true; // 是否有删除权限，默认为true（为了测试）
    ...
    function registerEventListeners() {
        ...
        if (addSpecBtn) {
            addSpecBtn.addEventListener('click', function() {
                ...
                // 检查属性名称是否已存在
                const exists = productAttributes.some(function(attr) {
                    return attr.name === specName;
                });
                
                if (exists) {
                    alert('属性名称已存在，请输入不同的名称');
                    return;
                }
        ...
        if (specDescriptionInput) {
            specDescriptionInput.addEventListener('change', function() {
                ...
                // 查找当前编辑的属性
                const attrIndex = productAttributes.findIndex(function(attr) {
                    return attr.name === currentEditingAttr;
                });
                
                if (attrIndex >= 0) {
                    // 更新属性说明
                    const oldValue = productAttributes[attrIndex].value;
                    productAttributes[attrIndex].value = newValue;
                    
                    // 更新UI
                    updateSpecUI(attrIndex, newValue);
                    ...
                    // 如果找不到当前编辑的属性，添加新规格
                    addSpec(currentEditingAttr, newValue);
    ...
    // 更新规格UI
    function updateSpecUI(index, newValue) {
        const specItems = document.querySelectorAll('.spec-item');
        if (index >= 0 && index < specItems.length) {
            const item = specItems[index];
            const valueCell = item.querySelector('div:nth-child(2)');
            if (valueCell) {
                const valueText = valueCell.querySelector('span');
                if (valueText) {
                    valueText.textContent = newValue || '';
                    valueText.className = 'text-red-600'; // 修改的内容显示为红色
                }
            }
        }
    }
    ...
    // 将当前显示的属性保存到历史记录中
    function saveCurrentAttributesToHistory() {
        productAttributes.forEach(function(attr) {
            if (!allAttributeRecords[attr.name]) {
                allAttributeRecords[attr.name] = [];
            }
            
            // 避免重复添加
            const exists = allAttributeRecords[attr.name].some(function(record) {
                return record.value === attr.value;
            });
            
            if (!exists && attr.value) {
                allAttributeRecords[attr.name].push({
                    value: attr.value,
    ...
    function initExistingSpecs() {
        ...
        existingSpecItems.forEach(function(item, index) {
            ...
            if (nameCell && valueCell) {
                ...
                // 添加到历史记录
                if (!allAttributeRecords[name]) {
                    allAttributeRecords[name] = [];
                }
                
                if (value) {
                    allAttributeRecords[name].push({
                        value: value,
                        timestamp: new Date().getTime()
                    });
                }
    ...
    function addControlButtons(item, name, value, index) {
        ...
        editBtn.addEventListener('click', function(e) {
            ...
            specNameInput.disabled = true; // 锁定属性名称，不能修改
            specDescriptionInput.value = '';
        ...
        deleteBtn.addEventListener('click', function(e) {
            ...
            if (confirm(`确定要删除属性"${name}"及其说明"${value}"吗？`)) {
                ...
                // 清空输入框
                specNameInput.value = '';
                specNameInput.disabled = false; // 解除锁定
                specDescriptionInput.value = '';
                currentEditingAttr = null;
            }
        });
    ...
    // 设置删除权限
    function setDeletePermission(hasPermission) {
        hasDeletePermission = hasPermission;
        
        // 更新所有删除按钮的状态
        const deleteButtons = document.querySelectorAll('.spec-item button:nth-child(2)');
    ...
    // 提交记录时调用，将当前规格保存到历史记录中
    function saveRecord() {
        // 保存当前属性到历史记录
        saveCurrentAttributesToHistory();
    ...
    return {
        ...
        addSpec: addSpec,
        getSpecs: getSpecs,
        getAllSpecsHistory: getAllSpecsHistory,
